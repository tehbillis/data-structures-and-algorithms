...
const addPurchases = (arr) => {
  return arr.reduce((total, purchase) => { return total + purchase.purchasePrice; }, 0);
};
...
const countNumberOfElements = (arr) => {
  return arr.reduce((length) => { return length+1; }, 0);
};
...
const calculateAverage = (arr) => {
  let sum = arr.reduce((sum, num) => {sum = sum + num; return sum;}, 0);
...
const extractChildren = (arr) => {
  let filteredArr = arr.filter(character => {
    if (character.name.includes('a')) {
      return character;
    }
  });

  return filteredArr.reduce((children, character) => {
    if(character.children) {
      character.children.forEach(child => children.push(child));
    }
    return children;
  }, []);
};
